write_assay_tsv <- function(output_dir, input_csv, ridx, category = c("bacteria", "enzyme", "community"), source = NA){
  input_assay <- read.csv(input_csv)
  ASSAY <- data.frame(matrix(ncol = 16, nrow = nrow(input_assay)))
  colnames(ASSAY) <- c("AIDX", "RIDX", "ASSAY_DESCRIPTION", "ASSAY_TYPE", "ASSAY_ORGANISM", "ASSAY_STRAIN", "ASSAY_TAX_ID",
                       "ASSAY_SOURCE", "ASSAY_TISSUE", "ASSAY_CELL_TYPE", "ASSAY_SUBCELLULAR_FRACTION", "TARGET_TYPE",
